...
def fact_table(
    ...
    # Gộp Order và Order Items thành bảng gốc
    Union = Union_order.join(Union_orderitems, "order_id", "inner")
    
    # --- SỬA LỖI ĐỊNH DẠNG NGÀY THÁNG ---
    # Tạo một cột mới chỉ chứa Ngày (YYYY-MM-DD) cắt bỏ giờ phút giây để khớp với dim_date
    Union = Union.withColumn("order_purchase_date", to_date(Union["order_purchase_timestamp"]))

    fact_table = (
        Union.join(dim_order, on=["order_id"], how="inner")
        .join(dim_product, on="product_id", how="inner")
        .join(dim_customer, on="customer_id", how="inner")
        .join(dim_seller, on="seller_id", how="inner")
        # Chuyển sang left join để không làm mất đơn hàng nếu đơn đó thiếu thông tin payment/review
        .join(silver_cleaned_payment, on="order_id", how="left")
        .join(silver_cleaned_order_review, on="order_id", how="left")
        # Khớp trường ngày đã được loại bỏ giờ phút giây (order_purchase_date) với full_date của dim_date
        .join(
            dim_date,
            Union["order_purchase_date"] == to_date(dim_date["full_date"]),
            how="inner",
        )
        .select(
            [
    ...
    # Tính toán row count thực tế của DataFrame
    actual_rows = fact_table.count()
    ...
    return Output(
        ...
        metadata={
            ...
            "column_count": len(fact_table.columns), # Đổi tên key hiển thị cho đúng bản chất
        },
    )
# def fact_table(
#     context,
...
def dim_date(context, silver_date: DataFrame):
    ...
    with get_spark_session(config, str(context.run.run_id).split("-")[0]) as spark:
        ...
        date_tuples = [(str(d),) for d in date_list]
